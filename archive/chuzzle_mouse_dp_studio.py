...
from chuzzle_mouse_dp import DpMouseSolver, FileScoreResult, GRID_SIZE, ScoredSolution, DEFAULT_NEXT_PUZZLE_TARGETS
...
def derive_step_views(solution: ScoredSolution) -> list[StepView]:
    ...
    for i, move in enumerate(solution.move_data):
        ...
        view = StepView(
            ...
            cumulative_cost=move.total_distance_to_here,
        )
        views.append(view)
...
def offset_segment(
    ...
    if dist <= EPSILON or total_slots <= 1:
        return start, end
    ...
    return (sx + px * amount, sy + py * amount), (ex + px * amount, ey + py * amount)
...
class StudioApp:
    def __init__(self, folder: str = DEFAULT_FOLDER):
        ...
        window_w = min(1600, max(1160, screen_w - 40))
        window_h = min(900, max(700, screen_h - 80))
    ...
    def _solver_supports_extended_options(self) -> bool:
        try:
            sig = inspect.signature(self.solver.score_file)
        except (TypeError, ValueError):
            return False
        params = sig.parameters
        return (
            "start_mouse_position" in params
            and "end_positions" in params
            and "end_next_puzzle" in params
        )
    ...
    def _build_right_panel(self, parent: ttk.Frame) -> None:
        ...
        self.steps_tree = ttk.Treeview(
            steps_frame,
            columns=("step", "move", "grp", "click", "release", "mv", "drag", "cumul"),
            show="headings",
            height=14,
        )
        for col, text, width in [
    ...
    def _update_active_options_summary(self) -> None:
        solver_mode = "extended" if self._solver_supports_extended_options() else "basic"
        text = (
    ...
    def _open_options_window(self) -> None:
        ...
        ttk.Label(
            solver_box,
            text="Custom point format: x,y\nMultiple end points: x,y; x,y; x,y",
            justify=tk.LEFT,
        ).grid(row=5, column=0, columnspan=2, sticky="w", pady=(8, 0))

        if not self._solver_supports_extended_options():
            ttk.Label(
                solver_box,
                text="Current chuzzle_mouse_dp.py does not expose start/end solver options yet.\nView settings still work. Solver will ignore start/end presets until the solver file is updated.",
                justify=tk.LEFT,
                foreground="#7c2d12",
    ...
    def _load_puzzle_file(self, filename: str) -> None:
        ...
        try:
            if self._solver_supports_extended_options():
    ...
    def _populate_steps_tree(self) -> None:
        ...
        for step in self.step_views:
            click = f"({step.click_down[0]:.0f},{step.click_down[1]:.0f})"
            release = f"({step.release[0]:.0f},{step.release[1]:.0f})"
    ...
    def _draw_board_background(self) -> None:
        ...
        self.canvas.create_rectangle(left - 20, top - 20, right + 190, bottom + 95, fill="#f8fafc", outline="")
        ...
        self.canvas.create_text(
            ...
            bottom + 22,
            anchor="w",
            text="Solid colored arrows = actual drag actions",
            font=("Segoe UI", 9),
            fill="#4b5563",
        )
        self.canvas.create_text(
    ...
    def _draw_header_metrics(self, solution: ScoredSolution) -> None:
        ...
        for i, (label, value, value_color) in enumerate(metrics):
            ...
            self.canvas.create_text(left + 12, y + 35, anchor="w", text=value, font=("Segoe UI", 13, "bold"), fill=value_color)
    ...
    def _draw_drag_paths(self) -> None:
        ...
        for step in self.step_views:
            sx, sy = board_to_canvas(*step.click_down)
            ex, ey = board_to_canvas(*step.release)
            (sx, sy), (ex, ey) = offset_segment((sx, sy), (ex, ey), *slot_lookup.get(step.step_index, (0, 1)))
            ...
            self.canvas.create_line(sx, sy, ex, ey, fill=outline, width=width + 3, arrow=tk.LAST, arrowshape=(16, 20, 7))
            self.canvas.create_line(sx, sy, ex, ey, fill=color, width=width, arrow=tk.LAST, arrowshape=(16, 20, 7))
        ...
        if self.current_step_index is not None and 0 <= self.current_step_index < len(self.step_views):
            ...
            self.canvas.create_rectangle(box_x, box_y, box_x + 100, box_y + 132, fill="#ffffff", outline="#d0d7de", width=2)
            details = [
                f"Step: {step.step_index + 1}",
                f"Token: {step.token}",
                f"Group: {step.group_id}",
                f"Click: ({step.click_down[0]:.0f}, {step.click_down[1]:.0f})",
                f"Release: ({step.release[0]:.0f}, {step.release[1]:.0f})",
                f"Gap: {step.move_distance:.3f}",
                f"Drag: {step.drag_distance:.3f}",
            ]
            for i, line in enumerate(details):
                self.canvas.create_text(box_x + 10, box_y + 14 + i * 17, anchor="w", text=line, font=("Segoe UI", 9), fill="#374151")
