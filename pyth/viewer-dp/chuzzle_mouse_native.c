#include "chuzzle_mouse_native_api.h"
...
static void chuzzle_zero_solution(ChuzzleNativeSolution* solution) {
    if (solution == NULL) {
        return;
    }
...
static TransitionChoice chuzzle_optimize_release_to_point(const MoveCandidate* candidate, ChuzzlePoint target) {
    ...
    transition.drag_distance =
        chuzzle_distance(candidate->click_down, candidate->lock_point)
        + chuzzle_distance(candidate->lock_point, transition.release);
    transition.move_to_next_distance = chuzzle_distance(transition.release, target);
    transition.total_cost = transition.drag_distance + transition.move_to_next_distance;
    return transition;
}
...
static void chuzzle_free_token_storage(char (*tokens)[CHUZZLE_MOVE_TOKEN_CAPACITY], char* normalized_move_string) {
    free(tokens);
    free(normalized_move_string);
}
...
int chuzzle_solve_sequence_utf8(
    ...
    char* normalized_move_string = NULL;
    ...
    chuzzle_zero_solution(out_solution);

    result = chuzzle_tokenize_move_string(move_string, &tokens, &token_count, &normalized_move_string);
    if (result != CHUZZLE_OK) {
        chuzzle_free_token_storage(tokens, normalized_move_string);
        return result;
    }
    ...
    if (sequence_len == 1) {
        ...
        out_solution->move_string = normalized_move_string;
        normalized_move_string = NULL;
        ...
        out_solution->steps = (ChuzzleNativeStep*)calloc(1, sizeof(*out_solution->steps));
...

        {
            ...
            ChuzzleNativeStep* step = &out_solution->steps[0];
    ...
    steps = (ChuzzleNativeStep*)calloc(sequence_len, sizeof(*steps));
    if (steps == NULL) {
        result = CHUZZLE_ERR_INVALID_ARGUMENT;
        goto cleanup;
    }

    {
        ...
        for (move_index = 0; move_index < sequence_len; ++move_index) {
            ...
            ChuzzleNativeStep* step = &steps[move_index];
            TransitionChoice transition;
...
void chuzzle_free_solution(
    ...
    free(solution->steps);
    chuzzle_zero_solution(solution);
}
