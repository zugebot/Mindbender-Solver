...
public:
    typedef void (*toDepthFuncPtr_t)(const Board &, std::vector<HashMem> &, HashMem::HasherPtr);
    ...
    MU static void getDepthPlus1BufferedFunc(const std::string& root_path, const std::vector<Board>& boards_in, std::vector<Board>& board_buffer, int depth);
};
